...
const UserTable: React.FC = () => {
  const { currentUserType } = useAuth(); // Get current user type from context
  ...
  const fetchManagers = async () => { // Fetch managers
    try {
      ...
      <div className="container mx-auto px-4 py-6" style={{ width: 1000, marginTop: 80, marginLeft: "350px" }}>
        ...
        {isCreateModalOpen && (
          <CreateUserModal
            ...
            onUserCreated={() => {
              fetchUsers(); // Refresh user list after creating user
              fetchManagers(); // Refresh managers to include any changes
            }}
            managers={managers}
            currentUserType={currentUserType} // Pass current user type
          />
        )}
