...
const UserTable: React.FC<{ managerId: number | null }> = ({ managerId }) => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      console.log(`Fetching users for managerId: ${managerId}`);
      const response = await fetch(`http://localhost:8000/users/executives?managerId=${managerId}`);
      ...
      const executiveUsers = data.filter(user => user.usertype === "EXECUTIVE");
        ...
        <table className="w-full border-collapse bg-white shadow-lg rounded-lg">
          <thead>
            <tr>
              ...
              {managerId && <th className="border p-3">Manager ID</th>}
          ...
          <tbody>
            {filteredUsers.map((user) => (
              <tr key={user.id}>
                ...
                {managerId && <td className="border p-3">{getManagerName(user.managerId)}</td>}
                <td className="border p-3 text-center">
                  <button onClick={() => handleEdit(user)} className="text-blue-500 hover:text-blue-700 mr-2">
                    <FaEdit />
                  </button>
                  <button onClick={() => handleDelete(user.id, user.username)} className="text-red-500 hover:text-red-700">
                    <FaTrash />
                  </button>
                </td>
              </tr>
